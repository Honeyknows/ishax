...
# Force cleanup Wazuh service and directory
Write-Host "    Force deleting WazuhSvc..." -ForegroundColor DarkGray
sc.exe delete WazuhSvc > $null 2>&1
...
$installDir32 = Join-Path $ProgramFilesX86 "ISHA-X"
$installDir64 = Join-Path $env:ProgramFiles "ISHA-X"

foreach ($dir in @($installDir32, $installDir64)) {
    if (Test-Path $dir) {
        Remove-Item -Path $dir -Recurse -Force -ErrorAction SilentlyContinue
        if (Test-Path $dir) {
            Write-Warn "Could not delete $dir entirely (files may be locked)."
        } else {
