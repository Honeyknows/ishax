...
.SYNOPSIS
    ...
    Installs Sysmon 15.21 + Wazuh Agent 4.8.0.
...
.NOTES
    ...
    Wazuh 4.8.0: https://packages.wazuh.com/4.x/windows/wazuh-agent-4.8.0-1.msi
#>
...
Write-Step "Step 2.2: Enable Process Creation Command Line Logging"
...
Write-OK "Process Creation Command Line Audit enabled (EID 4688 CommandLine)."
...
# -- active-response: Required for ISHAX Kill Switch (isolate.ps1 / unisolate.ps1)
# Do NOT disable this, or the admin isolation feature will fail silently.
# FIX #27: Changed from yes back to no.
$confRaw = $confRaw -replace "(<active-response>[\s\S]*?<disabled>)(yes)(</disabled>)", '${1}no${3}'
Write-OK "active-response enabled (required for kill switch)"
