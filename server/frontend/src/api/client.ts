...
export const api = {
  ...
  triggerTestAlert: () =>
    http.post<{ status: string; alert_ids: number[] }>('/test/trigger-alert').then(r => r.data),
  ...
  getTimeline: (params: { host_id: string; hours?: number; since_epoch?: number }) =>
  ...
  /** Delete a host completely from tenant database. */
  deleteHost: (hostId: string) =>
    http.delete<{ status: string; host_id: string }>(`/hosts/${encodeURIComponent(hostId)}`).then(r => r.data),
  ...
  downloadAgent: async (): Promise<void> => {
    ...
    a.download = match ? match[1] : 'ISHAX_Setup.bat'
    a.href = url
