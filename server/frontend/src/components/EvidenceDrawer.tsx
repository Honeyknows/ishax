        ...
        <div className="scroll-y" style={{ flex: 1 }}>
          ...
          >
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
              <span className="tag">{evidence.completeness?.level || 'unknown'}</span>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, minmax(0,1fr))', gap: 6 }}>
              {[
                ['Proc', evidence.counts?.processes ?? 0],
                ['Net', evidence.counts?.network ?? 0],
                ['File', evidence.counts?.files ?? 0],
                ['Reg', evidence.counts?.registry ?? 0],
                ['AMSI', evidence.counts?.amsi ?? 0],
              ].map(([label, value]) => (
