...
export default function ThreatHunt({ initialHost }: { initialHost?: string | null }) {
  ...
  const [centerView, setCenterView] = useState<'tree' | 'blast' | 'evidence' | 'chains'>('chains')
  ...
  const handleSelectAlert = useCallback(async (alert: Alert) => {
    ...
    try {
      ...
      setCenterView(prev => prev === 'chains' ? 'tree' : prev)
    } catch {
        ...
        {selectedAlert || centerView === 'chains' ? (
          <>
            ...
            <div style={{ flex: 1, overflow: 'hidden', display: 'flex' }}>
              <ErrorBoundary key={centerView} fallbackTitle={`${centerView.toUpperCase()} Panel Error`}>
                {centerView === 'tree' && (
