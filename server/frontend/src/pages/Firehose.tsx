...
import { Terminal, Pause, Play, Trash2, Download, Filter, AlignJustify, Code, ArrowDown } from 'lucide-react'
...
export default function Firehose() {
  ...
  // Reset stream delta pointers and scroll state on filter change
  useEffect(() => {
  ...
  const loadEvents = useCallback(async () => {
    ...
    try {
      const since = lastEpochRef.current
      ...
      if (data.events && data.events.length > 0) {
        let maxEpoch = lastEpochRef.current
        for (const e of data.events) {
          if (e.wazuh_ts_epoch && e.wazuh_ts_epoch > maxEpoch) {
            maxEpoch = e.wazuh_ts_epoch
          }
        }
        lastEpochRef.current = maxEpoch

        if (isInitialLoadRef.current || since === 0) {
          ...
          setEvents(prev => {
            ...
            const newDeltas = data.events.filter(e => !existingIds.has(e.id))
            if (newDeltas.length === 0) return prev
            return [...prev, ...newDeltas].slice(-500)
          })
        }
      }
    } catch {
      // Backend polling error catch
    } finally {
      setLoading(false)
    }
  ...
  // Auto-scroll to bottom whenever new events arrive if not scrolled up
  useEffect(() => {
      ...
      <div style={{
        ...
        <div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-3)', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Event Types</div>
            <button
              type="button"
              onClick={() => setTypes(selectedTypes.size === ALL_EVENT_TYPES.length ? new Set() : new Set(ALL_EVENT_TYPES))}
              style={{ background: 'none', border: 'none', color: '#6366f1', fontSize: 11, fontWeight: 600, cursor: 'pointer', padding: 0 }}
            >
              {selectedTypes.size === ALL_EVENT_TYPES.length ? 'Clear All' : 'Select All'}
            </button>
          </div>
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, marginLeft: 24 }}>
            ...
            <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
              <Button variant="ghost" size="sm" onClick={() => scrollToBottom(true)} icon={<ArrowDown size={12} />} title="Scroll to latest logs">
                Bottom
              </Button>
          ...
          <div className="empty-state">
            ...
            {selectedTypes.size === 0 ? (
              <p style={{ color: '#F97316', fontWeight: 600 }}>
                All Event Types are unchecked! Click <strong>Select All</strong> or check Process/Network/System on the left panel.
              </p>
            ) : (
