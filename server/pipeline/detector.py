...
def source_process_guid(ev: dict) -> str | None:
    ...
    # FIX #5: AMSI events — process_guid is already in ev["process_guid"] (normalized
    # by ingestor.normalise()). The old branch checked ev["system"] which is a raw-JSON
    # path that doesn't exist on the normalized dict. Use the correct normalized key.
    channel = (ev.get("channel") or "").lower()
    if channel == "ishax-amsi":
        # Try the AMSI JSON payload for a guid (older event shape fallback only)
...
# FIX #2: Lock protecting SIGMA_RULES clear+extend to prevent iterator/writer races.
_SIGMA_RULES_LOCK = threading.Lock()
# FIX #1: Track rules.db mtime so load_sigma_rules() is O(1) on every event
#         (only does real work when the DB file has changed).
_SIGMA_RULES_DB_MTIME: float = 0.0
_RULES_DB_PATH = Path(__file__).parent / "rules.db"
...
def load_sigma_rules():
    ...
    FIX #1: Only reloads when rules.db mtime changes — O(1) on every event.
    FIX #2: Uses _SIGMA_RULES_LOCK to protect list mutation.
    """
    global SIGMA_RULES, _SIGMA_RULES_DB_MTIME

    # FIX #1: Mtime guard — skip expensive compilation if file unchanged
    try:
        mtime = _RULES_DB_PATH.stat().st_mtime
    except OSError:
        return  # rules.db not yet created — keep current rules
    if mtime == _SIGMA_RULES_DB_MTIME:
        return  # No change — skip

    # Build new rule list outside the lock so the lock is held only briefly
    new_rules: list[dict] = []
        ...
        try:
            ...
            for rule in col.rules:
                for query in backend.convert_rule(rule):
                    ...
                    new_rules.append({"rule": rule, "sql": sql, "rule_id": rule_id})
        except Exception as exc:
            print(f"[WARN] Sigma rule compile failed ({rule_id}): {exc}", flush=True)

    # FIX #2: Atomically swap under lock — iterator in run_rules() takes a snapshot first
    with _SIGMA_RULES_LOCK:
        SIGMA_RULES.clear()
        SIGMA_RULES.extend(new_rules)
        _SIGMA_RULES_DB_MTIME = mtime

    print(f"[INFO] Loaded {len(new_rules)} Sigma rules from rules.db.", flush=True)
...
def _build_alert(
    ...
    # FIX #12: fired_at must be set from event time, not DB insert time.
    # schema.sql has DEFAULT (strftime('%s','now')) which is insert time — wrong
    # for backlogged events. Explicitly set it here from the event's Wazuh timestamp.
    fired_at = ev.get("wazuh_ts_epoch") or int(time.time())
...
def run_rules(con: sqlite3.Connection, ev: dict, rowid: int) -> list[dict]:
    ...
    """
    # FIX #1/#2: load_sigma_rules() now has an mtime guard (O(1) when unchanged)
    # and uses _SIGMA_RULES_LOCK internally. Call it here for hot-reload support.
    load_sigma_rules()
    # FIX #3: _purge_active_detections() was incorrectly called inside the per-group
    # loop in _run_merge(), causing O(N_groups) DELETE queries per merge call.
    # Moved here — one purge per run_rules() invocation is sufficient.
    _purge_active_detections(con)
    ...
    # ------------------------------------------------------------------
    # FIX #2: Take a snapshot of SIGMA_RULES under lock so the list cannot be
    # cleared mid-iteration by a concurrent load_sigma_rules() call.
    with _SIGMA_RULES_LOCK:
        rules_snapshot = list(SIGMA_RULES)
    for r in rules_snapshot:
        ...
        # Normalise technique (e.g. T1543.3 → T1543.003, T1003.1 -> T1003.001)
        # pySigma may strip leading zeros — re-add them dynamically for any sub-technique
        if technique:
            m_tech = re.match(r"^(T\d+)\.(\d+)$", technique, re.IGNORECASE)
            if m_tech:
                technique = f"{m_tech.group(1).upper()}.{int(m_tech.group(2)):03d}"
            else:
