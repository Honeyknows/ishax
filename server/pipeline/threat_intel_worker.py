...
from multi_tenant_manager import MASTER_DB_PATH, TENANT_DB_DIR
...
def main():
    print(f"[VT] worker started spacing={SPACING_SECONDS}s", flush=True)
    if not MASTER_DB_PATH.exists():
        print(f"[VT] {MASTER_DB_PATH} not found. Waiting...")
        while not MASTER_DB_PATH.exists():
            time.sleep(5)
    ...
    while True:
        try:
            with sqlite3.connect(MASTER_DB_PATH) as master_conn:
                ...
                tenants = master_conn.execute("SELECT id, db_filename FROM tenants").fetchall()
                
            did_work_overall = False
            for t in tenants:
                tenant_db = TENANT_DB_DIR / t["db_filename"]
                if not tenant_db.exists():
                    continue
                try:
                    with connect(tenant_db) as conn:
                        did_work = process_once(conn)
                        if did_work:
                            did_work_overall = True
                except Exception as e:
                    print(f"[ERROR] VT worker failed on tenant {t['id']}: {e}")
                    
            time.sleep(SPACING_SECONDS if did_work_overall else 5)
        except Exception as e:
            print(f"[ERROR] VT worker main loop: {e}")
            time.sleep(5)
