...
class TenantManager:
    ...
    def __init__(self):
        ...
        # Thread-local storage for DB connections per thread: self._local.conns = { tenant_id -> sqlite3.Connection }
        self._local = threading.local()
        # Cache: { agent_id -> tenant_id } to avoid master.db lookup on every log line
        self._agent_cache: dict[str, str] = {}
        self._lock = Lock()
        # Track last re-check time for UNKNOWN agents (TTL = 5s).
        self._unknown_retry: dict[str, float] = {}
        # Batched commit tracker for update_agent_last_seen()
        self._last_master_commit: float = 0.0
        self._prime_cache()
    ...
    def purge_tenant(self, tenant_id: str):
        ...
        with self._lock:
            ...
            if row:
                ...
                # Close thread-local connection if present in calling thread
                if hasattr(self._local, "conns"):
                    con = self._local.conns.pop(tenant_id, None)
                    if con:
                        try:
                            con.close()
                        except Exception:
                ...
                if db_path.exists():
                    try:
                        db_path.unlink()
                        # Also attempt to delete -shm and -wal
                        Path(str(db_path) + "-shm").unlink(missing_ok=True)
                        Path(str(db_path) + "-wal").unlink(missing_ok=True)
                    except PermissionError:
                        print(f"[TenantManager] Warning: Could not delete {db_path} because it is locked by another process. It is orphaned.", flush=True)
    ...
    def register_agent(self, tenant_id: str, agent_id: str, agent_name: str = "") -> dict:
        ...
        with self._lock:
            ...
            if existing:
                ...
                # FIX #20: Warn clearly when agent_id is already owned by a different tenant.
                # Previously this silently re-routed logs to the wrong tenant DB.
                if existing_dict.get("tenant_id") != tenant_id:
                    print(
                        f"[WARN][register_agent] Agent {agent_id!r} is already registered "
                        f"to tenant {existing_dict.get('tenant_id')!r}. Ignoring re-registration "
                        f"request from tenant {tenant_id!r}. Revoke the agent first before "
                        f"re-assigning it to a new tenant.",
                        flush=True,
                    )
    ...
    def update_agent_last_seen(self, agent_id: str):
        ...
        Throttled to once every 10 seconds per agent with an immediate commit
        to prevent holding open write transactions on master.db.
        """
        now = time.time()
        cache_key = f"seen:{agent_id}"
        if now - self._unknown_retry.get(cache_key, 0.0) < 10.0:
            return
        self._unknown_retry[cache_key] = now
        with self._lock:
            try:
                self._master.execute(
    ...
    def get_tenant_db_by_agent(self, agent_id: str) -> sqlite3.Connection | None:
        ...
        FIX #4: UNKNOWN sentinel now has a 60-second TTL. Previously every log
        line from an unregistered agent triggered a master.db SELECT, defeating
        the purpose of the cache.
        """
        tenant_id = self._agent_cache.get(agent_id)
        if tenant_id == "UNKNOWN":
            # FIX #4: Only re-check master.db after the TTL expires (5 s).
            last_check = self._unknown_retry.get(agent_id, 0.0)
            if time.time() - last_check < 5.0:
                return None  # Still within cooldown — skip DB hit
            # TTL expired — clear sentinel and fall through to DB lookup below
        ...
        if not tenant_id:
            ...
            if row:
                tenant_id = row[0]
                self._agent_cache[agent_id] = tenant_id
                self._unknown_retry.pop(agent_id, None)  # clear any stale TTL entry
            else:
                self._agent_cache[agent_id] = "UNKNOWN"
                self._unknown_retry[agent_id] = time.time()  # FIX #4: record re-check time
                return None
    ...
    def _get_pooled_connection(self, tenant_id: str) -> sqlite3.Connection | None:
        ...
        if tenant_id in self._local.conns:
        ...
        self._local.conns[tenant_id] = con
        return con
    ...
    def add_allowed_user(self, email: str, password: str = "", added_by: str = "admin", note: str = "") -> dict:
        ...
        # FIX #8: An empty password causes verify_user_password() to return False
        # (the `not row["password_hash"]` guard), permanently blocking login.
        # Disallow empty passwords at creation time.
        if not password:
            raise ValueError(
                "A password is required. Users cannot be created without one. "
                "Set a password so the user can log in."
            )
