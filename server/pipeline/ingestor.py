...
if os.getenv("INGESTOR_LOG_FILE"):
    log_file = open(os.getenv("INGESTOR_LOG_FILE"), "a", encoding="utf-8")
    sys.stdout = log_file
    sys.stderr = log_file
...
def _resolve_archives_path() -> str:
    ...
    candidates = [
        # Relative project bind-mount (server/wazuh/logs/archives/archives.json)
        str(Path(__file__).parent.parent / "wazuh" / "logs" / "archives" / "archives.json"),
        # Docker bind-mount (/tmp/wazuh_logs)
...
def normalise(raw: dict) -> dict | None:
    ...
    out_dict = {
        ...
        "command_line":    ci_get(edata, "CommandLine", "commandLine", "ProcessCommandLine", "ScriptBlockText", "scriptBlockText"),
        ...
        "process_path": ci_get(edata, "Image", "image", "NewProcessName", "imagePath", "ImagePath"),
...
class DB:
    ...
    def insert_process_node(self, ev: dict):
        try:
            # FIX #6: Use ev["process_guid"] (already normalized by normalise())
            # instead of re-parsing raw_json. raw_json may be None in multi-tenant
            # mode, causing silent no-ops when pguid comes back as None.
            pguid = ev.get("process_guid")
            if not pguid: return

            # For the remaining fields, try normalized ev first, then fall back
            # to raw_json for fields not promoted by normalise().
            ppguid = ev.get("parent_process_guid")
            pid    = ev.get("pid")  # not in normalized schema — use raw_json fallback
            image  = ev.get("process_path") or "Unknown"
            cmd    = ev.get("command_line") or ""
            user   = ev.get("username") or ""
            if pid is None:
                # Fall back to raw_json only for PID which normalise() doesn't extract
                try:
    ...
    def terminate_process_node(self, ev: dict):
        try:
            # FIX #6: Use ev["process_guid"] instead of re-parsing raw_json.
            pguid = ev.get("process_guid")
            if not pguid: return
            ts = ev.get("wazuh_ts")
    ...
    def insert_process_edge(self, ev: dict):
        try:
            ...
            if eid == 3:
                edge_type = "network"
                ip   = ev.get("destination_ip") or ""
                port = ev.get("destination_port") or ""
                target_label = f"{ip}:{port}" if ip else "Unknown IP"
            elif eid in (11, 23):
                edge_type    = "file"
                target_label = ev.get("target_filename") or ""
            elif eid in (12, 13, 14):
                edge_type    = "registry"
                target_label = ev.get("registry_path") or ev.get("target_object") or ""
    ...
    def prune_old_telemetry(self, retention_days: int = RETENTION_DAYS):
        ...
        # FIX #13: Also protect events whose process_guid is linked to a live alert.
        # The original query only checked alert_event_links. Events related via
        # process_edges (but not directly linked to an alert) were deleted,
        # breaking future evidence-graph traversal for those alerts.
        self.con.execute(
            """
            DELETE FROM events
            WHERE wazuh_ts_epoch < ?
              ...
              AND (
                  process_guid IS NULL
                  OR process_guid = ''
                  OR process_guid NOT IN (
                      SELECT DISTINCT source_process_guid FROM alerts
                      WHERE source_process_guid IS NOT NULL
                        AND source_process_guid != ''
                  )
              )
            """,
            (cutoff,),
    ...
    if path == "stdin":
        ...
        while True:
            ...
            proc = subprocess.Popen(
                ["docker", "exec", container_name, "tail", "-F", "-n", "100", "/var/ossec/logs/archives/archives.json"],
            ...
            print("[WARN] docker exec tail disconnected. Restarting in 2s...", flush=True)
            time.sleep(2)
    else:
        while True:
            try:
                with open(path, "r", encoding="utf-8", errors="replace") as fh:
                    fh.seek(offset)
                    while True:
                        # FIX #16: Save byte position BEFORE readline.
                        # If the line is partial (no trailing \n), seeking back
                        # using `fh.tell() - len(line.encode())` is wrong for
                        # multi-byte UTF-8 sequences read with errors="replace":
                        # the byte count of the decoded string may differ from
                        # the bytes actually consumed. Saving pre-read position
                        # is always correct regardless of encoding.
                        line_start = fh.tell()
                        line = fh.readline()
                        if not line:
                        ...
                        if not line.endswith("\n"):
                            ...
                            time.sleep(POLL_INTERVAL)
                            fh.seek(line_start)  # FIX #16: seek to pre-read position
                            continue
                        offset = fh.tell()
...
def process_line(db: DB, line: str, new_offset: int, run_rules) -> bool:
    ...
    if MULTI_TENANT:
        ...
        if not agent_id:
            # No agent ID in log — cannot route. Preserve in a quarantine table.
            if new_offset:
                db.set_offset(new_offset)
            return False
        ...
        if tenant_con is None:
            # Unknown or revoked agent — preserve for later recovery instead of silently dropping.
            agent_name = str(raw.get("agent", {}).get("name", "") or "")
            try:
                db.con.execute(
                    "INSERT INTO unknown_agents(agent_id, agent_name, first_seen_epoch, last_seen_epoch, source_line, status) VALUES (?, ?, ?, ?, ?, 'pending') "
                    "ON CONFLICT(agent_id) DO UPDATE SET agent_name=excluded.agent_name, last_seen_epoch=?, source_line=excluded.source_line",
                    (agent_id, agent_name, int(time.time()), int(time.time()), line, int(time.time())),
                )
                db.con.commit()
            except Exception as exc:
                print(f"[MULTI-TENANT] failed to record unknown agent {agent_id}: {exc}", flush=True)
            print(
                f"[MULTI-TENANT] RECORDED_UNKNOWN: agent_id={agent_id!r} agent_name={agent_name!r}"
                f" — pending tenant sync",
                flush=True,
            )
            if new_offset:
                db.set_offset(new_offset)
        ...
        # FIX #9: DB.__new__ bypasses __init__, so .path must be set explicitly.
        # Previously set to str(tenant_con) which is "<sqlite3.Connection object at 0x...>"
        # — meaningless as a path. Look up the real DB filename from master.db.
        try:
            _tid = mgr._agent_cache.get(agent_id, "")
            _row = mgr._master.execute(
                "SELECT db_filename FROM tenants WHERE id = ?", (_tid,)
            ).fetchone()
            from multi_tenant_manager import TENANT_DB_DIR
            target_db.path = str(TENANT_DB_DIR / _row["db_filename"]) if _row else f"tenant:{_tid}"
        except Exception:
            target_db.path = f"tenant:{agent_id}"  # safe fallback
