...
# FIX #11: Singleton connection — DDL is run once at startup, not on every call.
# Previously get_rules_db() opened a new connection AND re-executed the full DDL
# (CREATE TABLE IF NOT EXISTS, CREATE INDEX IF NOT EXISTS) on every invocation.
# With the bug where load_sigma_rules() was called per-event, this meant tens of
# thousands of DDL re-executions per minute.
#
# All callers do `con.close()` in finally blocks. _NoCloseProxy intercepts close()
# and makes it a no-op so the singleton connection stays alive.

class _NoCloseProxy:
    """Wraps sqlite3.Connection but ignores close() — used for the singleton pattern.
    All attribute access is forwarded to the underlying connection."""
    __slots__ = ("_conn",)
    ...
    # sqlite3.Connection context-manager support
    def __enter__(self):
...
def get_rules_db() -> _NoCloseProxy:
    """Return a thread-local rules.db connection (thread-safe, DDL run once)."""
    _ensure_ddl()
    if not hasattr(_local_rules, "conn") or _local_rules.conn is None:
        conn = sqlite3.connect(_DB_PATH, check_same_thread=True, timeout=30.0)
        conn.row_factory = sqlite3.Row
        _local_rules.conn = conn
    return _NoCloseProxy(_local_rules.conn)
...
def record_rule_hit(rule_id: str) -> None:
    ...
    FIX #24 (documentation): The formula `CAST(hit_count + 1 AS REAL)` in the
    SET clause uses SQLite's evaluation semantics where all SET expressions see
    the *original* row values before any assignment in the same statement.
    So `hit_count + 1` = OLD_hit_count + 1 = NEW_hit_count after the increment.
    noise_score therefore always reflects the current hit_count. No off-by-one.
    """
    now = int(time.time())
    con = get_rules_db()
    try:
