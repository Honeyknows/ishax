...
_COOLDOWN_TTL: float = 3600.0  # FIX #15: entries older than 1 h are cleaned up


def _cleanup_download_cooldown():
    """FIX #15: Remove stale cooldown entries to prevent unbounded dict growth.
    Called before each download check."""
    now = time.time()
    expired = [e for e, ts in _download_cooldown.items() if now - ts > _COOLDOWN_TTL]
    for e in expired:
        del _download_cooldown[e]
...
def build_process_tree(conn: sqlite3.Connection, root_guid: str, host_id: str | None = None) -> Dict[str, Any]:
    # Resolve actual host_id from root process_node if available
    root_node_row = conn.execute("SELECT host_id FROM process_nodes WHERE process_guid = ? LIMIT 1", (root_guid,)).fetchone()
    if root_node_row and root_node_row["host_id"]:
        host_id = root_node_row["host_id"]
...
def get_health(request: Request):
    ...
    # FIX #14: When unauthenticated, return ONLY a minimal status response.
    # The original code fell back to the admin's edr.db and leaked pipeline stats
    # (event counts, agent names, last alert details) to unauthenticated callers.
    try:
        conn, _ = get_tenant_db_for_request(request)
        authenticated = True
    ...
    if not authenticated:
        # start_local.ps1 checks status in ["healthy", "degraded", "empty", "ok"]
        # This minimal response satisfies that check without leaking data.
        return {"status": "healthy", "utc": utc_now(), "db_exists": True}

    # Authenticated: return full pipeline stats for this tenant only
    with conn:
...
def query_ai(payload: AIQuery, request: Request):
    ...
    with conn:
        ...
        return {
            # FIX #18: Clarify this is rule-based keyword matching, not an LLM.
            "mode": "rule-based-analyst",
            "disclaimer": (
                "This analyst uses keyword matching and pre-built queries against live "
                "SQLite data. It is NOT an AI/LLM. Open-ended questions return "
                "pre-formatted summaries based on what the database contains."
            ),
            "answer": answer,
            "citations": citations,
...
def get_rules(request: Request):
    ...
    rules = []
    for r in rows:
        mitre = json.loads(r.get("mitre_techniques") or "[]")
...
@app.post("/test/trigger-alert")
def trigger_test_alert(request: Request):
    conn, tenant = get_tenant_db_for_request(request)
    import time, sys, os
    ts = int(time.time())
    wazuh_id = f"test_payload_{ts}"
    agent_name = tenant.get("display_name") or "ishax-tenant_8d8b6763-TUF17"
    parent_guid = f"{{{ts-1:08x}-0000-0000-0000-000000000000}}"
    proc_guid = f"{{{ts:08x}-1111-2222-3333-444444444444}}"
    
    ev = {
        "wazuh_id": wazuh_id,
        "wazuh_ts": utc_from_epoch(ts),
        "wazuh_ts_epoch": ts,
        ...
        "command_line": "powershell.exe -w hidden -enc JABzAD0ATgBlAHcALQBPAGIAagBlAGMAdAAgAE4AZQB0AC4AVwBlAGIAQwBsAGkAZQBuAHQACgA=",
    ...
    with conn:
        # 1. Insert Event
        rowid = conn.execute(
            'INSERT INTO events (wazuh_id, wazuh_ts, wazuh_ts_epoch, agent_name, channel, event_id, process_path, image_path, command_line, process_guid, raw_json_original, raw_json_normalized, raw_json) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
            (ev['wazuh_id'], ev['wazuh_ts'], ev['wazuh_ts_epoch'], ev['agent_name'], ev['channel'], ev['event_id'], ev['process_path'], ev['image_path'], ev['command_line'], ev['process_guid'], '{}', '{}', '{}')
        ).lastrowid
        
        # 2. Insert Alert
        aid = conn.execute(
            "INSERT INTO alerts (fired_at, rule_id, rule_name, mitre_technique, severity, summary, event_id_fk, source_process_guid, source_agent_name, source_type, source_channel, source_event_id, source_wazuh_ts_epoch) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (ts, "c13454b6-ce2c-4903-875c-ce3d17db2388", "Suspicious PowerShell Execution (AMSI+Cmdline Dual-Layer)", "T1059.001", "medium", "Suspicious PowerShell Execution (AMSI+Cmdline Dual-Layer) | [no_amsi_corroboration — AMSI bypass possible or session down]", rowid, ev["process_guid"], ev["agent_name"], "powershell", ev["channel"], ev["event_id"], ts)
        ).lastrowid
        inserted_ids.append(aid)

        # 3. Insert Process Nodes (Parent -> Target)
        conn.execute(
            "INSERT OR REPLACE INTO process_nodes (process_guid, parent_process_guid, image, command_line, pid, user_name, host_id, start_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (parent_guid, None, "C:\\Windows\\explorer.exe", "explorer.exe", 1024, "SYSTEM", agent_name, utc_from_epoch(ts-5))
        )
        conn.execute(
            "INSERT OR REPLACE INTO process_nodes (process_guid, parent_process_guid, image, command_line, pid, user_name, host_id, start_time) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (proc_guid, parent_guid, ev['image_path'], ev['command_line'], 4096, "SYSTEM", agent_name, ev['wazuh_ts'])
        )

        # 4. Insert Evidence Artifacts into process_edges (File & Network)
        conn.execute(
            "INSERT OR REPLACE INTO process_edges (process_guid, host_id, edge_type, target_label, timestamp) VALUES (?, ?, ?, ?, ?)",
            (proc_guid, agent_name, "file", "C:\\Users\\Public\\malicious_payload.exe", ev['wazuh_ts'])
        )
        conn.execute(
            "INSERT OR REPLACE INTO process_edges (process_guid, host_id, edge_type, target_label, timestamp) VALUES (?, ?, ?, ?, ?)",
            (proc_guid, agent_name, "network", "185.220.101.5:4444", ev['wazuh_ts'])
        )

        conn.commit()
    return {"status": "alert_triggered", "alert_ids": inserted_ids}


@app.get("/hosts")
def get_hosts(request: Request):
    conn, tenant_id = get_tenant_db_for_request(request)
    mgr = _get_saas_manager()
    master_last_seen = {}
    if mgr:
        try:
            m_rows = mgr._master.execute(
                "SELECT agent_name, last_seen_at FROM agents WHERE is_revoked = 0"
            ).fetchall()
            for r in m_rows:
                if r["agent_name"] and r["last_seen_at"]:
                    master_last_seen[r["agent_name"]] = r["last_seen_at"]
        except Exception:
            pass
    ...
    with conn:
        ...
        for row in rows:
            ...
            ev_last = row["last_seen_epoch"] or 0
            m_last = master_last_seen.get(aname, 0)
            final_last = max(ev_last, m_last)
            hosts_res.append({
                "host_id": aname,
                "pc_name": aname,
                "registered_at": utc_from_epoch(row["first_seen_epoch"]),
                "last_seen": utc_from_epoch(final_last),
            })

        return {"hosts": hosts_res}
...
async def delete_host(host_id: str, request: Request):
    conn, tenant_id = get_tenant_db_for_request(request)
    with conn:
        conn.execute("DELETE FROM events WHERE agent_name = ?", (host_id,))
        conn.execute("DELETE FROM alerts WHERE agent_name = ?", (host_id,))
        conn.execute("DELETE FROM process_edges WHERE host_id = ?", (host_id,))
        conn.commit()
    ...
    if mgr:
        try:
            row = mgr._master.execute(
                "SELECT agent_id FROM agents WHERE agent_name = ? OR agent_id = ?", 
                (host_id, host_id)
            ).fetchone()
            if row:
            ...
            logger.warning(f"Error revoking agent {host_id}: {e}")
...
@app.get("/timeline")
def get_timeline(request: Request, host_id: str = "all", hours: int = 2, since_epoch: float = 0.0):
    hours = max(1, min(int(hours or 2), 168))
    cutoff = int((datetime.datetime.utcnow() - datetime.timedelta(hours=hours)).timestamp())
    conn, _ = get_tenant_db_for_request(request)
    
    host_pattern = host_id.strip() if host_id and host_id != "all" else "all"
    ...
    with conn:
        ...
        if since_epoch <= 0:
            alert_rows = conn.execute(
                ...
                WHERE a.fired_at >= ? AND (e.agent_name LIKE '%' || ? || '%' OR e.agent_id = ? OR ? = 'all')
                ORDER BY a.fired_at DESC
                LIMIT 50
                """,
                (cutoff, host_pattern, host_pattern, host_pattern),
            ).fetchall()
            for row in alert_rows:
                events.append({
                    ...
                    "wazuh_ts_epoch": row["fired_at"],
                })

        if since_epoch > 0:
            event_rows = conn.execute(
                """
                SELECT wazuh_id, event_id, channel, raw_json_original, raw_json_normalized, raw_json, wazuh_ts, wazuh_ts_epoch
                FROM events
                WHERE wazuh_ts_epoch > ? AND (agent_name LIKE '%' || ? || '%' OR agent_id = ? OR ? = 'all') AND (event_id IS NULL OR event_id != 5449)
                ORDER BY wazuh_ts_epoch ASC
                LIMIT 200
                """,
                (int(since_epoch), host_pattern, host_pattern, host_pattern),
            ).fetchall()
        else:
            event_rows = conn.execute(
                """
        ...
        for row in event_rows:
            ...
            evt_type = "system"
            if channel == "ishax-amsi" or eid in (4103, 4104, 4105, 4106):
                evt_type = "amsi"
            ...
            events.append({
                ...
                "label": label,
                "event_timestamp": row["wazuh_ts"],
                "wazuh_ts_epoch": row["wazuh_ts_epoch"],
...
def get_sysmon_xml():
    sysmon_path = os.path.join(ENDPOINT_SRC, "sysmon_config.xml")
    try:
...
def get_ossec_conf(request: Request):
  ...
  <client>
    ...
    <time-reconnect>15</time-reconnect>
...
@app.get("/deploy/assets/{filename}")
def get_deploy_asset(filename: str):
    valid_assets = ["Sysmon64.exe", "amsi_watcher.exe", "tailscale-setup.exe", "wazuh-agent-4.8.0-1.msi"]
    if filename not in valid_assets:
        raise HTTPException(404, "Asset not found")
    filepath = os.path.join(ENDPOINT_SRC, filename)
    if not os.path.isfile(filepath):
        raise HTTPException(404, "Asset file missing on server")
...
def get_install_script(request: Request):
    ...
    script = rf"""<#
.SYNOPSIS
    ISHA-X EDR Auto-Enrollment Deployment Script
#>
param(
...
try {{
    ...
    if (-not $wazuhService -or -not (Test-Path $wazuhExe)) {{
        if ($wazuhService) {{ sc.exe delete WazuhSvc > $null 2>&1 }}
        Write-Host "[*] Downloading Wazuh Agent MSI from local server..." -ForegroundColor Cyan
        Invoke-WebRequest -Uri "{base_url}/deploy/assets/wazuh-agent-4.8.0-1.msi" -OutFile "$env:TEMP\wazuh-agent.msi" -UseBasicParsing
        Write-Host "[*] Installing Wazuh Agent..." -ForegroundColor Cyan
        $process = Start-Process -FilePath "msiexec.exe" -ArgumentList "/i $env:TEMP\wazuh-agent.msi /q WAZUH_MANAGER=`"{host_ip}`" WAZUH_REGISTRATION_SERVER=`"{host_ip}`"" -Wait -NoNewWindow -PassThru
        if ($process.ExitCode -ne 0 -and $process.ExitCode -ne 3010) {{ throw "Wazuh MSI installation failed with exit code $($process.ExitCode)" }}
    }}
}} catch {{
...
# 3. Apply ossec.conf
try {{
    Write-Host "[*] Pulling Custom ossec.conf..." -ForegroundColor Cyan
    ...
    Write-Host "[X] Error downloading ossec.conf: $_" -ForegroundColor Red
    Exit
}}

# 4. Sysmon Installation
try {{
    Write-Host "[*] Downloading Sysmon Ruleset and Executable..." -ForegroundColor Cyan
    ...
    if ($sysmonService) {{
        ...
        Invoke-WebRequest -Uri "{base_url}/deploy/assets/Sysmon64.exe" -OutFile "$env:TEMP\Sysmon64.exe" -UseBasicParsing
        Start-Process -FilePath "$env:TEMP\Sysmon64.exe" -ArgumentList "-accepteula -i `"$localSysmonConfig`"" -Wait -NoNewWindow
    }}
...
# 5. AMSI Watcher Installation
try {{
    Write-Host "[*] Downloading and installing AMSI Watcher..." -ForegroundColor Cyan
    $InstallDir = "$env:ProgramFiles\ISHA-X"
    if (-not (Test-Path $InstallDir)) {{ New-Item -ItemType Directory -Path $InstallDir -Force | Out-Null }}
    
    Invoke-WebRequest -Uri "{base_url}/deploy/assets/amsi_watcher.exe" -OutFile "$InstallDir\amsi_watcher.exe" -UseBasicParsing
    
    # Ensure AMSI EventLog exists
    if (-not [System.Diagnostics.EventLog]::SourceExists("ISHAX-AMSI")) {{
        New-EventLog -LogName "ISHAX-AMSI" -Source "ISHAX-AMSI"
    }}
    
    $svcName = "ISHAXAmsiWatcher"
    $existing = Get-Service -Name $svcName -ErrorAction SilentlyContinue
    if (-not $existing) {{
        New-Service -Name $svcName -BinaryPathName "$InstallDir\amsi_watcher.exe" -DisplayName "ISHAX AMSI ETW Watcher" -StartupType Automatic | Out-Null
    }}
    Start-Service $svcName -ErrorAction SilentlyContinue
}} catch {{
    Write-Host "[X] Error during AMSI Watcher installation: $_" -ForegroundColor Red
}}

# 6. Wazuh Auto-Enrollment
try {{
    if ($TenantId) {{
        Write-Host "[*] Auto-Enrolling agent with tenant: $TenantId" -ForegroundColor Cyan
        $agentAuth = "C:\Program Files (x86)\ossec-agent\agent-auth.exe"
        $agentName = "ishax-$TenantId-$env:COMPUTERNAME"
        Start-Process -FilePath $agentAuth -ArgumentList "-m {host_ip} -A `"$agentName`"" -Wait -NoNewWindow
    }} else {{
        Write-Host "[!] No Tenant ID provided, skipping auto-enrollment." -ForegroundColor Yellow
    }}
}} catch {{
    Write-Host "[X] Auto-enrollment failed: $_" -ForegroundColor Red
}}
...
async def download_agent(request: Request):
    """
    Dynamically generates an ISHAX_Setup.bat file that triggers the PowerShell deployment script.
    """
    user = get_current_user(request)
    email = user["email"]
    tenant = user.get("tenant") or {}
    ...
    base_url = f"http://{host_ip}:{port}"
    if host_ip == "localhost":
        # Usually local deployment wants the actual IP
        import socket
        try:
            s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            s.connect(("8.8.8.8", 80))
...
:: ISHA-X EDR Auto-Enrollment Setup
echo [*] Initializing ISHA-X EDR Deployment...
set TENANT_ID={tenant_id}
powershell -ExecutionPolicy Bypass -NoProfile -Command "Write-Host '[*] Downloading deployment payload...'; Invoke-WebRequest -Uri '{base_url}/deploy/install.ps1' -OutFile '%TEMP%\\ishax_install.ps1' -UseBasicParsing; & '%TEMP%\\ishax_install.ps1' -TenantId '%TENANT_ID%'"
pause
"""
    return Response(
        content=bat_content,
        media_type="application/x-bat",
        headers={"Content-Disposition": f'attachment; filename="ISHAX_Setup_{email.split("@")[0]}.bat"'}
    )
...
async def admin_isolate_agent(agent_id: str, request: Request):
    ...
    # FIX #19: Prevent cross-tenant kill-switch abuse
    tenant = user.get("tenant") or {}
    tenant_id = tenant.get("id")
