...
function Stop-OrphanProjectProcesses {
    ...
    Get-CimInstance -Query $query -ErrorAction SilentlyContinue |
        Where-Object { $_.CommandLine -and (
            ...
            ($_.CommandLine -match "threat_intel_worker\.py") -or
...
Stop-ProcessTreeFromPidFile (Join-Path $PIPELINE_DIR "ingestor.pid") "Ingestor"
Stop-ProcessTreeFromPidFile (Join-Path $PIPELINE_DIR "threat_intel_worker.pid") "Threat Intel Worker"
Stop-ProcessTreeFromPidFile (Join-Path $BACKEND_DIR "backend.pid") "Backend API"
Stop-ProcessTreeFromPidFile (Join-Path $FRONTEND_DIR "frontend.pid") "Frontend UI"
Stop-OrphanProjectProcesses
...
Remove-Item (Join-Path $PIPELINE_DIR "threat_intel_worker.log") -Force -ErrorAction SilentlyContinue
...
$env:INGESTOR_LOG_FILE = $ingestorLog
Remove-Item Env:\EDR_DB_PATH -ErrorAction SilentlyContinue

$ingestor = Start-Process -FilePath "pythonw" `
...
Write-Step "Starting agent sync loop"
$syncLog = Join-Path $PIPELINE_DIR "sync_agents.log"
$syncPid = Join-Path $PIPELINE_DIR "sync_agents.pid"

$syncLoop = Start-Process -FilePath "pythonw" `
    -ArgumentList "sync_agents_loop.py" `
    -WorkingDirectory $PIPELINE_DIR `
    -WindowStyle Hidden `
    -PassThru `
    -RedirectStandardOutput $syncLog `
    -RedirectStandardError "$syncLog.err"

$syncLoop.Id | Out-File $syncPid -Encoding UTF8
Start-Sleep -Seconds 1
if ($syncLoop.HasExited) { Write-Warn "Agent sync loop exited early. Check $syncLog" }
else { Write-Ok "Agent sync loop started. PID: $($syncLoop.Id)" }

Write-Step "Starting Threat Intel Worker"
$tiLog = Join-Path $PIPELINE_DIR "threat_intel_worker.log"
$tiPid = Join-Path $PIPELINE_DIR "threat_intel_worker.pid"

$tiWorker = Start-Process -FilePath "pythonw" `
    -ArgumentList "threat_intel_worker.py" `
    -WorkingDirectory $PIPELINE_DIR `
    -WindowStyle Hidden `
    -PassThru `
    -RedirectStandardOutput $tiLog `
    -RedirectStandardError "$tiLog.err"

$tiWorker.Id | Out-File $tiPid -Encoding UTF8
Start-Sleep -Seconds 1
if ($tiWorker.HasExited) { Write-Warn "Threat Intel Worker exited early. Check $tiLog (it's okay if VT_API_KEY is unset)" }
else { Write-Ok "Threat Intel Worker started. PID: $($tiWorker.Id)" }

Write-Step "Starting backend API"
$backendLog = Join-Path $BACKEND_DIR "backend.log"
$backendPid = Join-Path $BACKEND_DIR "backend.pid"

$backend = Start-Process -FilePath "pythonw" `
